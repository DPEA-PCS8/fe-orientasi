...
const PksiDetailPage: React.FC = () => {
  const { id: pksiId } = useParams<{ id: string }>();
  const navigate = useNavigate();
  const location = useLocation();
  const nestedPksiInfo = (location.state as { nestedPksiInfo?: NestedPksiInfo } | null)?.nestedPksiInfo ?? null;
      ...
      <Box sx={{ px: { xs: 3, md: 4.5, xl: 6 }, py: 4, mx: 'auto' }}>
        ...
        {nestedPksiInfo?.isNested && (
          ...
          >
            ...
            <Box sx={{ flex: 1 }}>
              <Typography variant="subtitle2" sx={{ fontWeight: 600, color: '#047857', mb: 0.5 }}>
                Informasi PKSI Mengikuti Parent
              </Typography>
              <Typography variant="body2" sx={{ color: '#065f46', fontSize: '0.875rem', lineHeight: 1.6 }}>
                PKSI <strong>{nestedPksiInfo.nestedName}</strong> memiliki status{' '}
                <strong>"Dikerjakan Dengan Cara Lain"</strong> dan mengikuti monitoring dari PKSI parent:{' '}
                <strong>{nestedPksiInfo.parentName}</strong>. Detail yang ditampilkan di bawah adalah data
                monitoring dari PKSI parent tersebut.
              </Typography>
            </Box>
          </Box>
        )}
                  ...
                  {resolveSkpaCodes(pksiData.pic_satker_ba).length > 0 ? (
                    resolveSkpaCodes(pksiData.pic_satker_ba).map((skpa, idx) => (
                      <Chip
                        ...
                        size="small"
                        sx={{ bgcolor: '#DA251C', color: 'white', fontWeight: 500, fontSize: '0.75rem', borderRadius: '6px' }}
                      />
                    ))
                  ) : (
                  ...
                  {pksiData.child_pksi_list.map((child, index) => (
                    <Box
                      ...
                      sx={{
                        ...
                        '&:hover': { bgcolor: 'rgba(16, 185, 129, 0.08)', borderColor: 'rgba(16, 185, 129, 0.3)', transform: 'translateX(4px)' },
                      }}
                    >
                ...
                <Box sx={{ display: 'flex', flexDirection: 'column', gap: 3 }}>
                  <Box>
                    ...
                    {pksiFiles.filter(f => (f.file_type === 'T01' || !f.file_type) && f.is_latest_version !== false).length > 0 ? (
                        ...
                        {pksiFiles.filter(f => (f.file_type === 'T01' || !f.file_type) && f.is_latest_version !== false).map((file, index, arr) => (
                          ...
                          >
                            ...
                            <ListItemText
                                ...
                                <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                                  ...
                                  {file.version && file.version > 1 && (
                                    <Chip
                                      ...
                                      size="small"
                                      sx={{ height: 18, fontSize: '0.7rem', fontWeight: 600, bgcolor: '#059669', color: 'white' }}
                                    />
                                  )}
                                </Box>
                              }
                              ...
                              primaryTypographyProps={{ sx: { fontWeight: 500, color: '#1d1d1f', fontSize: '0.9rem' } }}
                            ...
                            <ListItemSecondaryAction>
                              {(file.content_type === 'application/pdf' || file.content_type?.startsWith('image/')) && (
                                <IconButton
                                  ...
                                  sx={{ color: '#0891B2', mr: 1, '&:hover': { bgcolor: 'rgba(8, 145, 178, 0.1)' } }}
                                  title="Lihat"
                                >
                      ...
                      <Box sx={{ p: 2, textAlign: 'center', borderRadius: '12px', bgcolor: 'rgba(245, 245, 247, 0.8)' }}>
                        <Typography variant="body2" sx={{ color: '#86868b' }}>
